...
export class AddListComponent implements OnInit {
  ...
  noCategoriesMessage: string | null = null;
  categoriesMessage: string | null = null;
  ...
  fetchAllCategories() {
    this.loading = true;
    this.noCategoriesMessage = null;
    this.apiService.getAllCategories().subscribe({
      next: (data) => {
        this.categories = Array.isArray(data) ? data : [];
        if (this.categories.length === 0) {
          this.noCategoriesMessage = 'لا يوجد فئات متاحة';
        }
        this.loading = false;
      },
      error: (error) => {
        console.error('خطأ في جلب كل الفئات:', error);
        this.noCategoriesMessage = 'فشل جلب الفئات';
        this.loading = false;
      },
    });
  }
